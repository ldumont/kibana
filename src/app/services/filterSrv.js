    ...
    this.toEjsObj = function (filter) {
      ...
      case 'range':
        return ejs.RangeFilter(filter.field)
          ...
          .to(filter.to);
      case 'querystring':
        return ejs.QueryFilter(ejs.QueryStringQuery(filter.query).analyzeWildcard(true));
      case 'field':
        return ejs.QueryFilter(ejs.QueryStringQuery(filter.field+":("+filter.query+")").analyzeWildcard(true)).cache(true);
      case 'terms':
